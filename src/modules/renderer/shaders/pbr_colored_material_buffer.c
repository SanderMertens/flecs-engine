...
static const char *kShaderSource =
    ...
    "  _pad : f32\n"
    "}\n"
    "@group(0) @binding(1) var<storage, read> material_buffer : array<GpuMaterial>;\n"
    ...
    "  @location(7) material_index : f32,\n"
    "  @location(8) _unused : f32\n"
    "};\n"
    "struct VertexOutput {\n"
    ...
    "};\n"
    "fn unpackColor(rgba : u32) -> vec4<f32> {\n"
    ...
    "}\n"
    "fn fetchMaterial(material_index : f32) -> GpuMaterial {\n"
    "  let material_count = arrayLength(&material_buffer);\n"
    "  if (material_count == 0u) {\n"
    "    return GpuMaterial(0xFFFFFFFFu, 0.0, 1.0, 0.0);\n"
    "  }\n"
    "  var index = u32(max(material_index, 0.0));\n"
    "  if (index >= material_count) {\n"
    "    index = material_count - 1u;\n"
    "  }\n"
    "  return material_buffer[index];\n"
    "}\n"
    ...
    "  let material = fetchMaterial(input.material_index);\n"
    ...
    "  out.color = unpackColor(material.color_rgba);\n"
    "  return out;\n"
    "}\n"
...
ecs_entity_t flecsEngineShader_pbrColoredMaterialBuffer(
    ecs_world_t *world)
{
    return flecsEngineEnsureShader(world, "PbrColoredMaterialBufferShader",
        &(FlecsShader){
