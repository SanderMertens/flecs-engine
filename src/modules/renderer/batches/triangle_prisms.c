...
static void flecsEngine_triangle_prisms_ensureCapacity(
...
redo: {
        ...
        while (ecs_query_next(&it)) {
            ...
            if ((ctx->batch.count + it.count) <= ctx->batch.capacity) {
                ...
                for (int32_t i = 0; i < it.count; i ++) {
                    ...
                    ctx->cpu_sizes[index].size.x = triangle_prisms[i].x;
                    ctx->cpu_sizes[index].size.y = triangle_prisms[i].y;
                    ctx->cpu_sizes[index].size.z = triangle_prisms[i].z;
                }
        ...
        if (ctx->batch.count > ctx->batch.capacity) {
            flecsEngine_triangle_prisms_ensureCapacity(
